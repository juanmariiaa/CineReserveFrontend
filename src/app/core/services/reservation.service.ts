...
export class ReservationService {
  ...
  getScreeningSeats(screeningId: number, timestamp?: number): Observable<any[]> {
  ...
  createCheckoutSession(reservationId: number, successUrl?: string, cancelUrl?: string): Observable<CheckoutSessionResponse> {
  ...
  getReservationStatus(reservationId: number): Observable<any> {
    return this.http.get<any>(`${environment.apiUrl}/reservations/${reservationId}/status`);
  }
}
